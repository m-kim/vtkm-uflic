...
/// CUDA contains its own random operations
///
template <typename T>
class DeviceAdapterRandomArrayImplementation<T, vtkm::cont::DeviceAdapterTagCuda>
{
public:
  VTKM_CONT
  ...
  inline __device__ T Set(vtkm::Id index, T a, T b) const
  {
    T* lockedValue = ::thrust::raw_pointer_cast(this->Portal.GetIteratorBegin() + index);
...
private:
  ...
  inline __device__ vtkm::Int64 vtkmSet(vtkm::Int32 * address,
                                        const vtkm::Id & idx,
                                        T a,
                                        T b) const
  {
